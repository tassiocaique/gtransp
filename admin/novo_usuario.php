 ...
 <body>
	...
	<div id="formulario">
		<form method="post" action="inserir_usuario.php">
			<?php
					if(isset($_GET['r'])) {
						$resultado = $_GET['r'];
						if ($resultado == "1") {
							echo "<div class='success'><center><h3>Usuário inserido com sucesso!</h3></center></div>";
						} else if ($resultado == "2") {
		...
		<h1>Inserir um novo usuário</h1>
			<label>
				<span>CPF (Login): </span>
				<input class="normal" type="text" name="login" id="login" onchange="javascript:validarCPF(this.value);" onkeypress="javascript: mascara(this, cpf_mask);"  maxlength="14"/>
				<p id="mensagem"></p>
			</label>
			<label>
				<span>Nome: </span>
				<input class="normal" type="text" name="nome"/>
			</label>
			...
			<label>
				...
				<input class="normal" type="text" name="departamento"/>
			</label>
