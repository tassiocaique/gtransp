		...
		<div id="formulario">
			<?php
				if(isset($_GET['r'])) {
					$resultado = $_GET['r'];
					if($resultado == "1") {
						echo "<div class='success'><h3>Arquivo publicado com sucesso!</h3></div>";
					} else {
						echo "<div class='fail'><h3>Ocorreu um erro!</h3></div>";
					} 
				}
			?>
			<h1>Upload de Arquivo de Receitual</h1>
			<h6>Se você deseja enviar um arquivo que não é referente a nenhuma <u>RECEITA</u>, por favor, retorne ao <a href="home_admin.php">Painel de Controle</a> e envie-o na seção correta. </h6>
			<form method="post" action="inserir_arquivo.php" enctype="multipart/form-data">
				...
				<label>
					<span>Gênero: </span>
					<input class="genero" type="text" readonly="readonly" name="genero" value="Receitas"/>
				</label>
				<label>
