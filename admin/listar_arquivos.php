		...
		<div id="table_content">
			<?php
				if(isset($_GET['r'])) {
					$resultado = $_GET['r'];
					if($resultado == "1") {
						echo "<div class='success'><h3>Arquivo excluído com sucesso!</h3></div>";
					} else {
			...
			<?php
				...
				$query = $db->query("SELECT * FROM $tabela WHERE `$tabela`.`visivel`=1 AND `$tabela`.`genero`!='Receitas' AND `$tabela`.`genero`!='Despesas'");
				...
				while ($conteudo = $query->fetch(PDO::FETCH_ASSOC)) {
					...
					echo "<td>".$autor."</td>";
					echo "<td><a href='editar_arquivo.php?ctl=".$conteudo['cod_controle']."'><img src='../imagens/edit.png'></a></td>";
