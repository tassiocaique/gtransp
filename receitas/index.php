		...
		<div id="pagina">
			...
			<?php
				...
				if(isset($_GET['pesquisa'])) {
					$busca = $_GET['pesquisa'];
					$query = $db->query("SELECT * FROM `$tabela` WHERE (`$tabela`.`visivel` = 1 AND `$tabela`.`genero`='Receitas') AND ((`$tabela`.`identificacao` LIKE '%".$busca."%') OR ('%".$busca."%'))  ORDER BY `$tabela`.`data_versao` DESC");
				} else {
					$query = $db->query("SELECT * FROM `$tabela` WHERE `$tabela`.`visivel` = 1 AND `$tabela`.`genero`='Receitas' ORDER BY `$tabela`.`data_versao` DESC");
				}
					...
					while ( $conteudo = $query->fetch(PDO::FETCH_ASSOC) ) {
						...
						echo "  <div id='noticia'>
								<div id='corpo'>
									<table>
									<tr>
									<td><div class='download'><a href='".$conteudo['caminho']."'><img src='../imagens/downloadreceitas.png'/></a></div></td>
									<td>
